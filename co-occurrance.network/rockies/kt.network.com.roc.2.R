...
setwd("/Users/BrownLab/Desktop/Students/Avery/projects/snow.bloom.2018/stats.analysis/kendall.tau.network/rockies")
...
taxa <- as.matrix(read.csv("all.lineage.rockies.csv", row.names = 1))
phylo.taxa <- tax_table(taxa)
phyloseq <- phyloseq(phylo.taxa)

#Read in snow community data matrix
abund <- as.matrix(read.csv("all.abund.rockies.csv", row.names = 1))
data <- t(abund)
...
otu <- c("AOtu0001")
title.name <- "Sanguina nivaloides (AOtu0001) (Rockies)"
...
#Plot Soletalia sp. (BOtu0001)
otu <- c("BOtu0001")
title.name <- "Soletalia sp. (BOtu0001) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
...
plot_network_2(otu.sub, physeq=phyloseq, color='Class', shape=NULL, title= title.name, abundance=abund,
               ...
               line_alpha=0.4, line_color=color, line_weight=TRUE, hjust = 0.3, 
               point_alpha=0.95, point_label_size=5, otu.center = otu)
dev.off()

#Plot Hymnobacter sp. Network (BOtu0002)
otu <- c("BOtu0002")
title.name <- "Hymnobacter sp. (BOtu0002) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
...
#Plot Ferruginibacter sp. Network (BOtu0003)
otu <- c("BOtu0003")
title.name <- "Ferruginibacter sp. (BOtu0003) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
...
#Plot Polaromonas sp. Network (BOtu0004)
otu <- c("BOtu0004")
title.name <-"Polaromonas sp.(BOtu0004) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
...
#Plot Rhizobacter sp. Network (BOtu0005)
otu <- c("BOtu0005")
title.name <- "Rhizobacter sp.(BOtu0005) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
...
#Plot Mycobotryomycetes unclassified (FOtu0006) Network
otu <- c("FOtu0006")
title.name <- "Mycobotryomycetes unclassified \n(FOtu0006) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
...
#Plot Mycosphaerella sp. (FOtu0007) Network
otu <- c("FOtu0007")
title.name <- "Mycosphaerella sp. (FOtu0007) (Rockies)"
otu.sub <- induced_subgraph(ig.thresh, vids = unlist(neighborhood(graph=ig.thresh, order=1, nodes=otu)))
transitivity(otu.sub, type = "localaverage", isolates = "zero")
average.path.length(otu.sub)
V(otu.sub)
